...
\textbf{[Emi: check again]} material ejected from Triton, %impactors,
...
Of these, 15 units, primarily on the leading hemisphere and in the south polar region, are referenced from \citet{CrowWillard2015}, while 10 units, mainly in the trailing hemisphere’s ridged plains, are adapted from \citet{Kirchoff2009}. The remaining units are newly delineated and proposed in this study. \\ %The remainder of units was derived here.\\
...
%two units in this study may be combined. Readers should regard the ``geologic unit" in this work, especially those subdivided in cratered plains, as a ``potential geological unit".
...
%when fitting for surface ages within different diameter ranges.
...
%appear younger than those relying solely on large craters. As such, for correct age estimation we need the crater production function (CPF).\\
...
\begin{equation}
    ...
    \label{equ:cpf_scale}
\end{equation}
%The polynomial fit is expressed as
%\begin{equation}
%    \log_{10} S(D_{\rm cr}) = \sum_{i=0}^{j} a_{i}\,\bigl[\log_{10} D_{\rm cr}\bigr]^i,
%    \label{equ:cpf}
%\end{equation}
...
%We could show that the crater size fequency distribution is overall steeper than previous assessments of the projectile populations implies. We also could show that the slope is steep throughout Enceladus' geological record, indicating that the steepness is not a result of massive secondary cratering contributions, and that the crater-forming projectile population has not recognizable changed through time.
